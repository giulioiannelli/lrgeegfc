...
verdict_layers:
  ...
  rho_split_coph_epi_excluded: SECONDARY_gray_cortical_dominant_core_not_carried (secondary tissue-characterization / localization layer parallel to C6, reported but not a primary headline; the PI directive 2026-06-05 "primary interpretive lens" elevation is demoted 2026-06-18, corrected + PI-reviewed. All numbers on the cophenetic object D_coph — raw |ImCoh| is the comparison baseline ONLY, never a result. CLAIM: the β cophenetic trace is gray-cortical-dominant and is NOT carried by the diseased seizure core. Evidence = SAME-GRAPH pair classes (no node-count confound), re-run at n=10 [Pat_15 SOZ-labelled → 17 epi nodes; audit_77 regenerated 2026-06-18, NO matched-strength sensitivity-flag flips vs n=9] + a new PAIR-COUNT control [audit_85 --mode pairclass, R=1000: is a class more trace-bearing than a random subset of the same number of pairs?]. SIGNIFICANT (cophenet): genuine hotspot = gray↔gray cortical coupling [gray_gray concentrated p_pair<0.001; wm_wm depleted p_pair=1.0; C6 gray/WM split]. EPI↔EPI core does NOT carry it: matched-strength weaken +0.142 p=0.161, pair-count trends depleted but NOT significant [cophenet p_pair=0.945, does not clear the 0.95 gate] → "core spared" is directional only, not a significant result. NONEPI +0.197 [p=0.005, persist] and CROSS interface +0.252 [p=0.010, persist] carry non-strength trace but are NOT pair-count hotspots. TWO WITHDRAWALS: [a] 'epi-exclusion STRENGTHENS the trace' [+0.221→+0.275] is a generic node-count effect [audit_85 --stratify epi, p_dec=0.390], NOT epi-specific; [b] 'CROSS interface is the strongest carrier' — interface not a pair-count hotspot [cophenet p_pair=0.093]. β verdict tag UNCHANGED — strong trace, both probes, earned at C3 on the full graph; this is an emphasis/localization layer. Source data/audit/epi_stratified/{cophenetic_cohort.csv, pairclass_decimation_cohort.csv, decimation_control_cohort.csv})
  rho_split_coph_wm_excluded: SECONDARY_survives_wm_exclusion_sharpening_NOT_wm_specific (C6, audit_83 2026-06-08 / amended audit_85 2026-06-12 → data/audit/wm_stratified/; the β cophenet trace SURVIVES the gray-only montage — obs +0.221→+0.324, full-cohort exclude_wm matched-strength p=0.053 MARGINAL but LO-Pat_15 p=0.037, gray_gray p=0.032, raw-β p=0.024 (small-n power crossing, preserved not lost). BUT the strengthening is NOT WM-specific: the random-node-decimation control (audit_85) reaches the same cohort ρ_split by removing any ~40 % of nodes (cohort p_dec=0.145, generic_nodecount); WM is neither carrying nor diluting the cophenet trace. The earlier "gray-resident-because-it-sharpens" framing is WITHDRAWN; load-bearing C6 claim = survival/robustness, not sharpening. SECONDARY/mechanistic per CONTROLS §C6, NOT a gate; β verdict tag UNCHANGED)
  grassmann_wm_excluded: SECONDARY_REPASSES_C3_GATE_under_wm_exclusion (C6, audit_86 2026-06-12; the β subspace trace RE-PASSES the LOCKED C3 cluster-extent mass gate on the gray-only montage — cluster_p_mass^wmX=0.005 STRONG, LOO-robust (0.005), matching the full-graph C3 verdict. Upgraded from the per-k Wilcoxon count (40→40, retained as co-statistic) to a re-pass of the actual locked gate. Flag B resolved)
  anatomy_cophenet: CONCENTRATES_IN_OFC_SYSTEM (2026-06-10 audited verdict — multi-region DK list retracted 2026-05-30; the brain-wide β cophenet trace CONCENTRATES in the ORBITOFRONTAL system above its per-patient baseline at the a-priori-system scale — concentration, not exclusive presence; matched-strength R=1000, rank concordance, canonical observed trace, BH q=0.009–0.013 across 9 systems within β in all 4 contact/shaft × incl/excl conditions, 4/5 implanted, leave-one-out + shaft-collapse robust, low-strength/non-hub, bilateral (peaks at system scale; washes out at lobe p≈0.29 / hemisphere p≈0.48-0.76; undersampled at region scale); Hip/MTL sub-threshold p≈0.4; occipital K=3 secondary; paralimbic-ring fails shaft-collapse. Source data/audit/localization_atlas/README.md; audit_83 + audit_92)
  anatomy_grassmann: NOT_LOCALIZED (2026-06-10 — the cross-phase Grassmann subspace probe does not localize in any band; the β localization is edge-cophenetic only. The 2026-05-19 "strong_localized" entry used the phase-AVERAGED participation ANCHOR, retracted 2026-05-30.)
sources:
...
revision_history:
  ...
  - 2026-06-05: **epi-exclusion ELEVATED from secondary mechanistic observation to PRIMARY INTERPRETIVE LENS** (PI directive 2026-06-05; pre-empts the "isn't this pathological-network reorganization?" reviewer attack). The β trace is a property of healthy network reorganization that the epileptic zone partly MASKS — strengthens under epi-exclusion and is carried by healthy cortex + the epi↔non-epi interface, not the diseased core. Updated Head (new epi-independence paragraph), frontmatter `verdict_layers.rho_split_coph_epi_excluded`, and §3.2.5 (header + status-elevation preamble + pair-class decomposition table + brutal-honesty caveats), citing audit_77 (`data/audit/epi_stratified/`, matched-strength p=0.0029, pair-class CROSS +0.273 / NONEPI +0.197 / EPI↔EPI n.s. +0.240 p=0.150 / epi_only −0.078) alongside the 2026-05-20 audit_68_beta numbers. **β verdict tag UNCHANGED** (`strong trace, both probes`, earned at C3 on the full graph); this is an emphasis/interpretation elevation only. Caveats kept: epi_only underpowered; pair-class null is global-rewiring not within-class (interface-carried = suggestive not airtight). See VERDICT_LEDGER.md 2026-06-05 revision.
  - 2026-06-08: added **C6 white-matter-exclusion** sensitivity (frontmatter `verdict_layers.rho_split_coph_wm_excluded` + `grassmann_wm_excluded`) from audit_83/84 (`data/audit/wm_stratified/`). WM (dominant-`Wm` contacts, 30–57 % of montage) removal PRESERVES/STRENGTHENS the β trace: cophenet obs +0.221→+0.324 (exclude_wm p=0.053 marginal but LO-Pat_15 0.037 / gray_gray 0.032 / raw-β 0.024); Grassmann per-k cells 40→40. The β trace is gray-matter-resident, NOT a white-matter recording artifact. SECONDARY/mechanistic per CONTROLS §C6, NOT a gate and NOT elevated to a primary interpretive lens (node-count caveat); **β verdict tag UNCHANGED** (`strong trace, both probes`). See VERDICT_LEDGER.md 2026-06-08 revision + `responses/2026-06-08_wm-exclusion-cascade.md`.
  - 2026-06-12: **resolved the two C6 honesty flags** (audit_85 + audit_86). Flag A (node-count): the cophenet "sharpening" is NOT WM-specific — the random-node-decimation control reaches the same cohort ρ_split by removing any ~40 % of nodes (cohort p_dec=0.145, generic_nodecount). WITHDRAW the "gray-resident-because-it-sharpens" framing; the β cophenet C6 claim is **survival/robustness, not sharpening** (the trace still clears matched-strength on the gray-only graph; WM is neither carrying nor diluting it). Flag B (Grassmann gate): the β subspace trace RE-PASSES the locked C3 cluster-extent mass gate under WM exclusion (audit_86 `cluster_p_mass^wmX=0.005`, strong, LOO-robust) — upgraded from the per-k count. Updated both `verdict_layers.*_wm_excluded` entries. **β verdict tag UNCHANGED.** See VERDICT_LEDGER.md 2026-06-12 amendment + `responses/2026-06-08_wm-exclusion-cascade.md`.
  - 2026-06-12 (same control, applied to C5 epi-X — the PRIMARY lens): `audit_85 --stratify epi` shows the β cophenet **subgraph** "strengthens under epi-X" (+0.221→+0.275) is ALSO generic node-count (cohort p_dec=0.390; epi-X drops only ~9.5% of nodes yet random removal reproduces it). The "strengthens under epi-X" rationale for the 2026-06-05 PRIMARY-lens elevation is WITHDRAWN; the lens RE-ANCHORS on the **pair-class** decomposition (CROSS interface + NONEPI cortex carry it, EPI↔EPI core n.s.) — same-graph, no node-count confound, unaffected. Updated `verdict_layers.rho_split_coph_epi_excluded`. **β verdict tag UNCHANGED** (earned at C3 full graph). ⚠ Flagged for PI review (touches the 2026-06-05 directive). Source `data/audit/epi_stratified/decimation_control_cohort.csv`.
  - 2026-06-18: **PI decision — epi-exclusion DEMOTED to SECONDARY** (tissue-characterization / sensitivity layer, parallel to C6; no longer the 2026-06-05 "primary interpretive lens"). Built the **pair-count control** (`audit_85 --mode pairclass`, R=1000: is a pair class more trace-bearing than a random subset of the same number of pairs?) and **regenerated audit_77 epi pair-class at n=10** (Pat_15 now SOZ-labelled, 17 epi nodes — no matched-strength flag flips; n=9 CSVs backed up). Corrected claim (cophenetic framework; raw |ImCoh| is the baseline only, never a result): **the β cophenetic trace is gray-cortical-dominant** (gray↔gray is the genuine pair-count hotspot, p_pair<0.001; wm↔wm depleted) and the **diseased seizure core does not carry it** (epi↔epi matched-strength `weaken`; pair-count trends depleted, cophenet p_pair=0.945, NOT significant → "core spared" is directional only). **Two withdrawals:** (a) "epi-exclusion strengthens the trace" = generic node-count (already flagged 2026-06-12); (b) "CROSS interface is the strongest carrier" — the interface is NOT a pair-count hotspot (cophenet p_pair=0.093). Updated frontmatter `rho_split_coph_epi_excluded`, Head epi paragraph, §3.2.5 (header + status + subgraph-table withdrawal banner + pair-class/pair-count table + caveats + provenance). **β verdict tag UNCHANGED.** Source `data/audit/epi_stratified/{cophenetic_cohort.csv, pairclass_decimation_cohort.csv}`; handoff `.agents/reports/2026-06-12_decimation-controls-handoff.md`.
---
...
β is the **only band whose post-task structural trace survives matched-strength surrogacy at both LRG probes** — the per-pair multiscale **cophenetic communication distance** `ρ_split^coph` on `D_coph = cophenet(UPGMA(D(τ_max)))` (cohort-paired Wilcoxon p = 0.005, effect-size ratio 23.7×, 7/10 patients above own surrogate; passes C1 split p=0.005, C2 drift p=0.014, C3 matched-strength p=0.005, C4 cross-probe 8/10) and the Grassmann chordal distance `d_G(k)` on the leading-k Laplacian eigenmode subspaces (cluster-extent permutation p = 0.005, audit_70; 29 contiguous matched-strength-significant k cells at k ∈ [27, 55], **strengthening to 36 cells at k ∈ [21, 56] under epileptic-zone exclusion**, 29/29 of the manuscript-window cells persist + 7 new emerge at lower k). The trace is ~~**localized to a distributed cortical network** (7 DK regions per probe pass A1+A3 join under matched-strength)~~ — the 7+7 multi-region DK lists were **retracted 2026-05-30/06-01** (they did not survive a signed, threshold-free test), but a sampling-conditioned re-analysis at the a-priori *anatomical-system* scale **localizes the β cophenet trace to ORBITOFRONTAL CORTEX** (2026-06-10): matched-strength R=1000, rank concordance, canonical observed trace, **BH q=0.009–0.013 across the 9 a-priori systems within β** in all four contact/shaft-collapsed × epi-include/exclude conditions, 4/5 implanted positive, leave-one-out + **shaft-collapse** robust, in low-strength/non-hub tissue (hippocampus/MTL is a real but sub-threshold hint, p≈0.4; the Grassmann subspace probe does not localize — the localization is edge-cophenetic; see §5) and **multiscale** (per-pair via cophenetic merge-height integration on `D_coph`; subspace via k-sweep on `U_k` — the two probes read different aspects of the same multiscale geometry). Verdict tag from `locked/VERDICT_LEDGER.md`: **`strong trace, both probes`** (locked 2026-05-18, revised 2026-05-19 for cluster-extent gate).

**Epileptic tissue (secondary tissue-characterization, parallel to C6; PI-reviewed 2026-06-18 — the 2026-06-05 "primary interpretive lens" framing is demoted): the β cophenetic trace is gray-cortical-dominant.** This secondary characterization shows the β trace is **healthy gray-cortical reorganization, not an epileptiform artefact** — stated in the LRG cophenetic framework (raw |ImCoh| is the comparison baseline only). The same-graph pair classes (no node-count confound), re-run at n=10 and put through a pair-count control (`audit_85 --mode pairclass`), show the genuine pair-count hotspot is **gray↔gray cortical coupling** (concentrated `p_pair<0.001`; white-matter↔white-matter depleted), and the **diseased seizure core does not carry the trace** (epi↔epi fails its matched-strength null, `weaken` p=0.16; it trends toward depleted but **not** significantly, cophenet `p_pair=0.945`). This pre-empts the "is this just pathological-network reorganization?" reading and lines up with the brain-wide-with-OFC-over-expression anatomy (OFC = healthy gray cortex). **Two earlier claims are withdrawn (brutal honesty):** the "epi-exclusion *strengthens* the trace" framing (a generic node-count effect, `audit_85 --stratify epi` p_dec=0.390) and the "epi↔non-epi interface is the strongest carrier" sub-claim (the interface is not a pair-count hotspot). This is an emphasis/localization layer; **the `strong trace, both probes` verdict is unchanged** and was earned at C3 on the full graph (see §3.2.5).
...
**Refined biological claim.** β-band cortical coupling reorganizes during the task and the reorganization is retained into the post-task resting state. The retention is multi-aspect (per-pair multiscale cophenetic communication geometry + global leading-mode subspace), multiscale (the cophenet integrates across N−1 dendrogram merge-height scales; Grassmann spans ~30 leading-eigenmode subspace dimensions), and — at the cophenet probe — **anatomically anchored in the orbitofrontal cortex** (OFC system, matched-strength R=1000, BH q≈0.01 within β, 4/5 implanted, leave-one-out + shaft-collapse robust, low-strength; audited 2026-06-10). The retention is not single-DK-region (multi-region DK lists retracted) and not Grassmann-subspace-localized; it is an **OFC-anchored, edge-cophenetic** β reorganization, with a sub-threshold hippocampal/medial-temporal hint.
...
#### 3.2.5 Epileptic tissue and the β cophenetic trace — gray-cortical-dominant; the diseased core does not carry it (audit_77 n=10 + audit_85 pair-count control, 2026-06-18; supersedes the 2026-05-20 epi-X subgraph framing)

**Status: a major component of the β result (PI directive 2026-06-05; corrected + PI-reviewed 2026-06-18).** The epileptic-tissue analysis is part of the main β trace result, not a side control: in the LRG cophenetic framework it establishes that the β cognitive task-trace is **gray-cortical-dominant and is not carried by the diseased seizure tissue**, not an epileptiform artefact — the direct answer to the obvious "is this just pathological-network reorganization?" reading. (All result statements are on the cophenetic object `D_coph`; raw |ImCoh| is the comparison baseline only.)

**What changed on 2026-06-18 (brutal honesty).** The original 2026-06-05 framing — "removing the diseased tissue makes the trace *stronger*" (cophenet +0.221 → +0.275) — is **WITHDRAWN**: that subgraph "strengthening" is a generic node-count effect, not epi-specific (random-node decimation reproduces it by dropping any ~9.5 % of nodes; `audit_85 --stratify epi`, cohort p_dec = 0.390). The claim is **re-anchored on the same-graph pair classes** (no node-count confound), re-run at the current **n = 10** (Pat_15 now SOZ-labelled, 17 epi nodes; `audit_77` regenerated 2026-06-18 — **no sensitivity-flag flips**) and put through a **new pair-count control** (`audit_85 --mode pairclass`). The corrected conclusion: **the β cophenetic trace is gray-cortical-dominant** — the significant pair-count hotspot is gray↔gray coupling (`p_pair < 0.001`), white-matter↔white-matter is depleted, and the **diseased seizure core does not carry the trace** (epi↔epi fails its matched-strength null, `weaken`; trends toward depleted but **not** significantly, `p_pair = 0.945`). This is consistent with the brain-wide-with-OFC-over-expression anatomy (§5; OFC is healthy gray cortex). **The β verdict tag (`strong trace, both probes`) is unchanged** — earned at C3 on the full graph (C1–C4 pass at full n=10 in §3.2; Grassmann passes independently in §3.3). This section frames and localizes the verdict; it does not change it.

The β cophenet epi-X audit was first run on 2026-05-20 (audit_68_beta), one day after the `VERDICT_LEDGER.md` lockdown; the 2026-05-19 brief frontmatter and the `Decision 4` ledger entry reflected the as-of-2026-05-19 "not run" state. It was confirmed and extended on 2026-06-05 by the six-view epi-stratified re-run (audit_77 → `data/audit/epi_stratified/`), which adds the pair-class decomposition. Both runs agree on the cohort headline; audit_77's matched-strength Wilcoxon (p=0.0029, 7/10 above own surrogate, LO-Pat_13 p=0.0059) is the current canonical number.
...
> ⚠ **WITHDRAWN (2026-06-18, node-count confound).** The "+0.221 → +0.275 strengthening" / "reinforced under epi-X" claim in this subgraph table is a **generic node-count effect, not epi-specific**: `audit_85 --stratify epi` reaches the same cohort ρ_split by dropping any ~9.5 % of RANDOM nodes (cohort p_dec = 0.390). This subgraph table is retained only as the historical sensitivity that motivated the re-anchoring; the load-bearing epi claim is the **same-graph pair-class + pair-count** analysis below. (The matched-strength gate the trace clears at C3 on the *full* graph is unaffected — that is not a subgraph contrast.)

**Reading (historical, node-count-confounded).** β cophenet under epi-X appears to strengthen the matched-strength gate (ratio 23.7× → 26.0×, p 0.005 → 0.003) and brings Pat_10 (the only LRG-anti patient at full β) into the trace direction; six of ten patients move strengthen-or-flip-positive, two weaken, Pat_15 invariant. **But per the banner above this apparent strengthening is not epi-specific** — read the pair-class section below for the corrected claim. The Grassmann epi-X broadening (§3.3, 29 → 36 cells) is a separate subspace probe and carries its own (also subgraph-based) caveat.
...
**Pair-class decomposition + pair-count control — the β cophenetic trace is gray-cortical-dominant (audit_77 n=10 regen 2026-06-18; audit_85 `--mode pairclass`).** The full-graph `ρ_split^coph` is restricted to node-pair classes — a **same-graph** analysis with no node-count confound (unlike the subgraph epi-X above). Two complementary same-graph nulls are now applied to each class:
- **matched-strength** (audit_77): does the class carry *non-strength* trace? (holds the global per-node strength sequence fixed)
- **pair-count** (audit_85 `--mode pairclass`, NEW 2026-06-18): is the class *more* trace-bearing than a **random subset of the same number of pairs** from the whole graph? `p_pair < 0.05` = concentrated (a genuine hotspot); `> 0.95` = depleted (below the graph average); else = indistinguishable from random pairs of that count. This is the within-class power null the earlier caveat flagged as missing — now run (R=1000).

Re-run at the current **n = 10** (Pat_15 SOZ-labelled 2026-06-11 → 17 epi nodes; epi↔epi / cross now defined for all 10). **No matched-strength sensitivity-flag flips** vs the n=9 run (Jun-5 CSVs backed up to `cophenetic_*_n9_2026-06-05.bak.csv`).

All numbers below are on the canonical LRG cophenetic object `D_coph` — **the raw-|ImCoh| substrate is the comparison baseline (§3.1), never cited as a result** (the pair-count control is computed on raw too, but only `D_coph` is reported here).

| Pair class (β cophenet `D_coph`) | n=10 `ρ_split^coph` (matched-strength p, flag) | pair-count `p_pair` (cophenet) | Reading |
|---|---|---|---|
| NONEPI↔NONEPI (healthy cortex) | +0.197 (0.005, persist) | 1.00 (depleted¹) | carries non-strength trace; not a hotspot |
| CROSS (epi↔non-epi interface) | +0.252 (0.010, persist) | 0.093 (≈ random, n.s.) | carries non-strength trace, but **NOT a pair-count hotspot** — "strongest carrier" withdrawn |
| EPI↔EPI (diseased core) | +0.142 (0.161, **weaken**) | 0.945 (trends depleted, **n.s.**) | **does NOT carry** (fails matched-strength); trends toward spared but not significant |

¹ nonepi↔nonepi is ~78 % of all pairs, so its pair-count verdict is near-deterministic and only weakly interpretable; the informative classes are the small/specific ones (cross, epi↔epi).

Source: `data/audit/epi_stratified/{cophenetic_cohort.csv, pairclass_decimation_cohort.csv}`. **Corrected conclusion (LRG cophenetic framework): the β cophenetic trace is gray-cortical-dominant.** The one genuine pair-count *hotspot* is **gray↔gray cortical coupling** (concentrated `p_pair < 0.001`; white-matter↔white-matter depleted `p_pair = 1.0`; from the C6 gray/WM split, `verdict_layers.*_wm_excluded`). The **diseased seizure core does not carry the trace** — epi↔epi fails its matched-strength null (`weaken`, p = 0.16) and is not a pair-count hotspot; it *trends* toward depleted (`p_pair = 0.945`) but does **not** reach significance, so "core spared" is directionally supported, **not** a significant result. Healthy cortex (nonepi) and the interface (cross) carry real (non-strength) trace but are not pair-count hotspots. With the brain-wide-with-OFC-over-expression anatomy (§5; OFC = healthy gray cortex), the β cophenetic trace is a **healthy gray-cortical reorganization, dominated by gray↔gray coupling and not carried by the diseased core or the white matter**.

**Brutal-honesty caveats.** (i) **All result statements here are in the LRG cophenetic framework**; the raw-|ImCoh| substrate is the comparison baseline (§3.1), never a result. (ii) The earlier "CROSS interface is the strongest carrier" claim is **WITHDRAWN** — surviving the matched-strength null (real, non-strength trace) is *not* being more trace-bearing than random pairs, and the cophenetic pair-count control puts the interface at the random-pair baseline (`p_pair = 0.093`, n.s.). (iii) "Core spared" is only a **trend on cophenet** (epi↔epi `p_pair = 0.945`, does not clear the 0.95 depleted gate) — report it as "the core does not carry the trace (matched-strength `weaken`) and trends depleted", not as a significant depletion. The significant, framework-clean result is **gray-dominance** (gray↔gray hotspot). (iv) The `epi_only` rebuilt subgraph (−0.078) is node-count-confounded *and* underpowered (6–30 nodes); dropped from the claim.
...
- Epi-stratified pair-class (audit_77, regenerated at n=10 2026-06-18): `scripts/01_compute/audit/audit_77_epi_stratified_cophenetic.py` → `data/audit/epi_stratified/{cophenetic_cohort.csv, cophenetic_per_patient.csv, README.md}` (n=9 backups `cophenetic_*_n9_2026-06-05.bak.csv`)
- **Pair-count control** (audit_85 `--mode pairclass`, 2026-06-18): `scripts/01_compute/audit/audit_85_wm_decimation_control.py --mode pairclass --stratify epi` → `data/audit/epi_stratified/{pairclass_decimation_cohort.csv, pairclass_decimation_per_patient.csv, README_pairclass.md}`
- **Node-count control** (audit_85 `--stratify epi`, 2026-06-12, withdraws the subgraph "strengthening"): `data/audit/epi_stratified/decimation_control_cohort.csv`
- Consolidated report: `.agents/reports/2026-06-05_epilepsy-headline-and-occult-node-marker.md`; handoff `.agents/reports/2026-06-12_decimation-controls-handoff.md`
...
## 5. Anatomical distribution — multi-region DK lists RETRACTED 2026-05-30; **β cophenet trace CONCENTRATES IN ORBITOFRONTAL CORTEX (audited verdict 2026-06-10)**

> ✅ **AUDITED VERDICT 2026-06-10 — the brain-wide β cophenet trace CONCENTRATES in ORBITOFRONTAL CORTEX (OFC system) above its per-patient baseline.** This is a *concentration*, not exclusive presence: the trace is carried by electrode pairs cohort-wide, and the audit demeans each patient's own cohort-wide trace before asking where it concentrates — so OFC is a reproducible **hotspot**, not a container (even within OFC the directional lean is only ≈0.59, ~41% of OFC pairs anti-trace). The 2026-06-01 "delocalized" blanket was over-aggressive: it judged regions against the full cohort (/10) when the denominator is patients *implanted* in the region, and gated on a cohort permutation test that is a structural false-negative at n=10/≤5-per-region. A sampling-conditioned re-analysis at the a-priori **anatomical-system** scale, under the **mandatory matched-strength surrogate** with rank concordance and a **canonical observed trace** (recomputed to match the surrogate construction), finds:
> - **OFC system clears in all four conditions** (contact / shaft-collapsed × epi-include / exclude): matched-strength **R=1000**, **BH q = 0.009–0.013** across the 9 a-priori systems within β; **4/5 implanted patients positive**, leave-one-out robust (LOO-min ≫ 0, survives dropping the anti-trace Pat_10), **shaft-collapse robust** (multi-shaft in the 2 strongest patients — Pat_05 3/4, Pat_06 5/6), in **low-strength / non-hub** tissue (strength dev −0.51).
> - **Scale-dependence (concentration peaks at the *system* scale).** Coarser pooling dilutes it and finer splitting undersamples it: **hemisphere** shows no lateralization (Left p≈0.48, Right p≈0.76 — OFC concentrates in *both* hemispheres, so the trace is **bilateral**); the **whole frontal lobe** washes out (p≈0.29, OFC diluted by the rest of frontal); **single DK regions** are undersampled (OFC sub-regions K=1–3, raw p≈0.001 but fail BH over 53 regions — the same wall α hits). The 9-way **system** scale is the only one both adequately sampled (OFC K=5) and multiplicity-survivable. (Hemisphere/lobe are descriptive label-shuffle; the matched-strength gate ran region/system/supersystem/paracore, and the gated coarse units point back to OFC.)
> - **Hippocampus/MTL: sub-threshold** (matched-strength p ≈ 0.4 at R=1000, leave-one-out fragile — Pat_02-driven). Strength-independent (survives residualization) but underpowered at n=5 — a real hint, **not** the localization.
> - **NOT localized:** "distributed paralimbic ring" (ring−OFC fails shaft-collapse, mode-unstable); **occipital** clears but on only **K=3** (low-coverage secondary, not headline); the cross-phase **Grassmann subspace** trace (single-probe; localization is edge-cophenetic).
> - **Six-rung control gate** (label-shuffle → matched-strength → strength-residual → within-β BH-FDR → per-patient/LOO → shaft-collapse) + **R=1000** + self-audit (canonical-trace fix; α demoted; multiplicity family = a-priori systems within β only, NOT bands/granularities/epi-modes). Source: `data/audit/localization_atlas/README.md`.
>
> **The multi-region DK *lists* in the tables below stay retracted** — the localization is at the **system** scale (OFC), not the 7-region list. The retraction banner below is retained as historical record of why the region lists were dropped.
>
> 🧭 **Interpretation (discussion-level — a hypothesis the localization *motivates*, NOT a tested result).** The task is **transitive inference (TI)**: in `task_learn` patients acquire an ordered set of premise relations (A>B, B>C, …) — a latent **hierarchy** — and in `task_test` they judge **novel non-adjacent pairs** (e.g. B>D) answerable only by **inferring from the learned ordering**. This is the canonical relational-memory / cognitive-map paradigm. Two pieces line up:
> - **Form of the measure mirrors form of the task.** The LRG cophenetic distance is the ultrametric of a *connectivity hierarchy*; the trace is a task-induced change in that hierarchy that persists offline into `rest_post`. The task is *learning a hierarchy*; the measure is *change in a hierarchy that sticks* — a structural, not just incidental, correspondence.
> - **The "where" matches the cognitive-map literature.** A β-band reorganization concentrating in **orbitofrontal cortex** connects directly to *OFC as a cognitive map of task/state space* (Wilson, Takahashi, Schoenbaum & Niv, 2014); the sub-threshold **hippocampus/MTL** hint is the expected relational-memory substrate of TI. Both are *predictions* the paradigm would make, met by data collected blind to it.
> - **Candidate framing:** *a human intracranial signature of the offline consolidation of a learned relational structure (cognitive map)*, with a possible clinical second life as a cognitive-network/epileptogenic marker.
>
> **Honesty flags (must travel with the interpretation):** (1) This is a *post-hoc* reading — the analyses were not designed around TI, and the OFC concentration is the audited result, the cognitive-map gloss is not. (2) The decisive functional test — **trace magnitude vs. per-patient TI behavioral performance** — has **not** been run; behavioral data is obtainable from collaborators but **not yet in the repo**. (3) Exact stimuli/timing remain undocumented; n=10, OFC implant coverage K=5, sEEG spatial sparsity. (4) The localization is an *above-baseline concentration*, bilateral and edge-cophenetic — not an exclusive or whole-region claim. Until (2) is run, this is a framing for the intro/discussion, not a result. See `memory/task_paradigm_transitive_inference.md`.
>
> ⚠️ **RETRACTED (historical; multi-region DK lists — superseded by the OFC system-level verdict above) — the β trace is anatomically DELOCALIZED, on BOTH probes.** The
...
| C5 epi-X (Grassmann) | broadens 29 → 36 cells on the reduced graph (subgraph-based, not pair-count-controlled) | `grassmann_regate_no_filter/contig_summary.csv` audit_67 |
| C5 epi (`ρ_split^coph`, pair-class + pair-count) | ✓ run at n=10 — **gray-cortical-dominant** (gray↔gray hotspot p_pair<0.001); diseased core does not carry it (epi↔epi `weaken`; trends depleted, n.s.); subgraph "strengthening" withdrawn (node-count) | `epi_stratified/{cophenetic_cohort,pairclass_decimation_cohort}.csv` |
