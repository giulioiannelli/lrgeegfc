...
verdict_layers:
  ...
  rho_split_coph_epi_excluded: PRIMARY_INTERPRETIVE_LENS_strengthens_under_epi_exclusion (epi-exclusion elevated from secondary mechanistic observation to a primary interpretive lens by PI directive 2026-06-05; the β trace is a property of healthy network reorganization that the epileptic zone partly MASKS — it persists AND strengthens when diseased tissue is excluded. audit_68_beta_epi_exclusion.py run 2026-05-20 + audit_77 epi-stratified cophenetic 2026-06-05 — cohort median +0.221 → +0.275, ratio 23.7× → 26.0×, one-sample Wilcoxon p = 0.003 / audit_77 matched-strength Wilcoxon p = 0.0029, 7/10 above own surrogate, LO-Pat_13 p = 0.0059, LOO max p = 0.006 Pat_02; 6 of 10 patients strengthen or flip positive under epi-X, including Pat_10 which inverts from −0.091 to +0.029. PAIR-CLASS decomposition (audit_77): trace CARRIED by the CROSS epi↔non-epi interface (+0.273, p=0.0098, 7/9) and NONEPI↔NONEPI healthy cortex (+0.197, p=0.0049, 7/10); EPI↔EPI diseased core is NON-SIGNIFICANT (+0.240, p=0.150) and the epi_only rebuilt subgraph WEAKENS (−0.078, p=0.715). ⚠ NODE-COUNT CORRECTION (audit_85, 2026-06-12): the SUBGRAPH "+0.221→+0.275 strengthening" is NOT epi-specific — random-node decimation reaches the same cohort ρ_split by removing the same ~9.5% of RANDOM nodes (cohort p_dec=0.390, generic_nodecount). The "strengthens under epi-X" rationale for the primary-lens elevation is WITHDRAWN; the lens must RE-ANCHOR on the PAIR-CLASS decomposition above (same-graph, no node-count confound, unaffected). The β verdict tag is UNCHANGED — strong trace, both probes, earned at C3 on the full graph; this is an emphasis/interpretation elevation only, now resting on pair-class not subgraph strengthening — ⚠ flagged for PI review re: the 2026-06-05 directive)
...
**Epi-independence is a primary interpretive lens (PI directive 2026-06-05).** The β trace is a property of **healthy network reorganization that the epileptic zone partly masks**: it not only survives but *strengthens* when the clinically labeled epileptogenic contacts are excluded (cophenet +0.221 → +0.275, p=0.003; Grassmann mass 69.76 → 89.04, p_mass=0.005, 29 → 36 cells), and the pair-class decomposition (audit_77) shows it is **carried by the healthy↔healthy cortex (+0.197) and the epi↔non-epi interface (+0.273), not by the diseased core** (epi↔epi non-significant +0.240, p=0.150; epi-only rebuilt subgraph weakens). This pre-empts the obvious "is this just pathological-network reorganization?" reading: it is not — excluding the pathology makes the cognitive task-trace cleaner. This is an emphasis/interpretation elevation; **the `strong trace, both probes` verdict is unchanged** and was earned at C3 on the full graph (see §3.2.5).
...
#### 3.2.5 Epileptogenic-zone exclusion — PRIMARY INTERPRETIVE LENS (audit_68_beta 2026-05-20 + audit_77 epi-stratified 2026-06-05)

**Status elevation (PI directive 2026-06-05).** Epi-zone exclusion is **no longer a secondary mechanistic observation** at β — it is elevated to a **primary interpretive lens**. The message it carries is the #1 reviewer-defense for the whole manuscript: the β cognitive task-trace is **a property of healthy network reorganization that the epileptic zone partly masks**, not an epileptiform artefact. Removing the diseased tissue makes the trace *cleaner and stronger* (cophenet +0.221 → +0.275; Grassmann 29 → 36 cells, mass 69.76 → 89.04), and the pair-class decomposition (audit_77, below) shows the trace lives in healthy cortex and at its interface with the epileptic zone — **not inside the diseased core**. **This elevates the EMPHASIS/INTERPRETATION only; the β verdict tag (`strong trace, both probes`) is unchanged** and was earned at C3 on the full graph (C1–C4 all pass at full n=10 in §3.2; the Grassmann probe passes independently in §3.3). A reader must not read this section as a change of verdict — it is a change of how the verdict is framed.
...
**Pair-class decomposition — the interface and healthy tissue carry the β trace (audit_77, 2026-06-05).** The full-graph `ρ_split^coph` null is re-evaluated restricted to three node-pair classes (holding the global per-node strength sequence fixed; median pair counts: nonepi↔nonepi 5565, cross 1050, epi↔epi 45):

| Pair class | Cohort-median `ρ_split^coph` (p) | n>surr | Reading |
|---|---|---|---|
| NONEPI↔NONEPI (healthy cortex) | **+0.197 (0.0049)** | 7/10 | **carries** the trace |
| CROSS (epi↔non-epi interface) | **+0.273 (0.0098)** | 7/9 | **carries** the trace (strongest class) |
| EPI↔EPI (diseased core, pair-restricted) | +0.240 (**0.150**) | 5/9 | **non-significant** — does not carry |
| epi_only (true LRG rebuilt on epi nodes) | −0.078 (0.715) | 2/9 | **weakens** — removing healthy tissue destroys the trace |

Source: `data/audit/epi_stratified/cophenetic_cohort.csv` (audit_77, six epi views × six bands × n=10, matched-strength R=200). **The β trace is carried by the healthy↔healthy and the epi↔healthy interface pair classes; the epi↔epi class is non-significant and the epi-only rebuilt subgraph weakens.** The β trace lives in healthy tissue and at its interface with the epileptic zone, not inside the diseased core — the mechanistic content of the "epi-exclusion strengthens" headline.

**Brutal-honesty caveats (bounding the interface claim).** (i) The **epi_only subgraph is underpowered** — 6–30 nodes (Pat_15 = 0), 15–435 pairs; matched-strength on such tiny graphs is near-degenerate, so the epi_only −0.078 / epi↔epi p=0.150 must be read as "no power to detect", not "proven absent". (ii) The **pair-class null is global-rewiring, not within-class**: it fixes the global strength sequence and asks whether class-restricted co-movement survives, but does not randomize *within* the class — so "interface-carried" is **suggestive, not airtight**; a within-class matched-strength null is the airtight version and has not been run. (iii) Class sizes are unequal (nonepi↔nonepi always has the most pairs → the most stable Spearman); the cross result (1050 pairs, p=0.0098) is not a size artefact but the comparison is not size-controlled.
