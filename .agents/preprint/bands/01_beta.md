...
verdict_layers:
  ...
  rho_split_coph_wm_excluded: SECONDARY_preserved_strengthens_under_wm_exclusion (C6, audit_83 2026-06-08 → data/audit/wm_stratified/; obs +0.221 → +0.324 on the gray-only montage, full-cohort exclude_wm matched-strength p=0.053 MARGINAL but LO-Pat_15 p=0.037, gray_gray pair-class p=0.032, and raw-β exclude_wm p=0.024 — a node-count power crossing (median 117→74 nodes), NOT WM-carried; the β trace is gray-matter-resident. SECONDARY/mechanistic per CONTROLS §C6, NOT a gate and NOT a primary interpretive lens; β verdict tag UNCHANGED)
  grassmann_wm_excluded: SECONDARY_unchanged_under_wm_exclusion (C6, audit_84 2026-06-08; per-k cohort Wilcoxon significant-cell count 40→40, longest run 29→21 — the β subspace trace fully survives the gray-only montage. NB per-k Wilcoxon basis, NOT the C3 cluster-extent mass gate, which was not re-run on the WM submatrix)
  anatomy_cophenet: LOCALIZES_TO_OFC_SYSTEM (2026-06-10 audited verdict — multi-region DK list retracted 2026-05-30; the β cophenet trace localizes to the ORBITOFRONTAL system at the a-priori-system scale — matched-strength R=1000, rank concordance, canonical observed trace, BH q=0.009–0.013 across 9 systems within β in all 4 contact/shaft × incl/excl conditions, 4/5 implanted, leave-one-out + shaft-collapse robust, low-strength/non-hub; Hip/MTL sub-threshold p≈0.4; occipital K=3 secondary; paralimbic-ring fails shaft-collapse. Source data/audit/localization_atlas/README.md; audit_83 + audit_92)
...
## 5. Anatomical distribution — multi-region DK lists RETRACTED 2026-05-30; **β cophenet trace LOCALIZES TO ORBITOFRONTAL CORTEX (audited verdict 2026-06-10)**

> ✅ **AUDITED VERDICT 2026-06-10 — the β cophenet trace localizes to ORBITOFRONTAL CORTEX (OFC system).** The 2026-06-01 "delocalized" blanket was over-aggressive: it judged regions against the full cohort (/10) when the localization denominator is patients *implanted* in the region, and gated on a cohort permutation test that is a structural false-negative at n=10/≤5-per-region. A sampling-conditioned re-analysis at the a-priori **anatomical-system** scale, under the **mandatory matched-strength surrogate** with rank concordance and a **canonical observed trace** (recomputed to match the surrogate construction), finds:
