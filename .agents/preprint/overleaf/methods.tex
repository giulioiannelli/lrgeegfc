...
\end{equation}
is the entropic susceptibility, which on networks with a discrete spectral gap exhibits peaks at the gap-induced crossover scales. The natural resolution window of the propagator is \([\tau_{\min}, \tau^{\ast}]\) with \(\tau_{\min} = 1/\lambda_{\max}\) the finest scale resolved by \(\propag(\tau)\) and \(\tau^{\ast} \approx 1/\lambda_{\rm gap}\) the coarsest scale before the trivial mode dominates. Throughout this work the analysis is anchored at the finest scale \(\tau = \tau_{\min} = 1/\lambda_{\max}\); the standard partition selector \(\Psi(n;\tau) = \mathcal{N}[\log_{10} \Delta_{n}(\tau) - \log_{10}\Delta_{n+1}(\tau)]\) of~\cite{villegas2025multiscale} does not isolate a privileged interior cut on the dense weight-heterogeneous \(\abs{\ImCoh}\) substrate, the spectrum is continuous, and the entropy/specific-heat reading of gap-induced crossover scales does not apply; for the same reason coarser \(\tau\) are not used here.
