...
| 06 | Why it matters: from detecting a change to characterizing it | NEW | [ ] | [x] | [x] | ♻ SETTLED 2026-07-16 (user, reasoned through the literature). Text-forward, **FOUR concept→citation anchors, each → a facet of our work**: (1) **DATA-EFFICIENCY** the "AI" kick — few-shot humans vs data-hungry ML (Liu–Behrens'19 opening line; Tsividis'17 dropped, LB covers it) → why study the human solution [a DATA claim, NOT "humans beat AI"/"AI solved it"]; (2) **COGNITIVE MAPS** — relations as a structured multi-scale map (Behrens'18) incl. SOCIAL networks (Son, Bhandari & FeldmanHall'21, user-requested) → hierarchical/multiscale lens; (3) **OFFLINE CONSOLIDATION** — built at rest, not at learning (Ellenbogen'07, behaviour-only) → encoding-vs-inference arc; (4) **REPLAY** — learned structure reactivated & reorganized at rest (Liu–Behrens'19; Wilson–McNaughton'94 backing) → the resting trace. Then **GAP→OUR MOVE** detect→CHARACTERIZE (anchor fig `slide06_detect_vs_characterize`) + **EPILEPSY CODA** Kramer & Cash'12 (multiscale network disorder, Jirsa'14 alt) → same lens same recording. Guardrail=matched-strength. Arrows point at what we STUDY not what we FOUND (no spoilers). FIXED: Anderson'72 dropped (slogan), Betzel not reused (slide 5), Ellenbogen behaviour-only, self-ref "seed questions" gone. Ellenbogen fig DROPPED (on disk). Left: Canva build (text-forward, one anchor fig); pin Tsividis'17 venue. |
| 07 | Diffusion / LRG | pg 7 · ~40% | [ ] | [ ] | [ ] | 3-axes CORRECTED (diffusion = topology) |
| 08 | Form Mirrors Form | pg 8 · ~55% | [x] | [x] | [x] | ✅ LOCKED M 07-11 (revised) — HERO = OUR 3D "form mirrors form" scene `pipeline_brain_tree_3d.png` (brain network at base → cophenetic tree rising above → diffusion streams; community-colour glue). Behrens/Park EXTERNAL figures DROPPED (user: "figure from the other papers completely wrong… not the one we're looking for"). On-slide text EXPANDED (7-line block). Optional small race-bracket callback + ≟. ⚠ REUSE: same fig is the slide-11 pipeline hero → bet=teaser/vision, 11=step-by-step build (or distinct camera); don't repeat unchanged. ⚠ black bg on light deck. Fixed 07→08 numbering lag. W: text+speech final. V: no cohort numbers. |
...
| 17 | COMPOUND CLOSER — take-homes · ongoing · outlook | NEW | [ ] | [x] | [x] | ♻ 2026-07-16 (user): old 17 take-homes + 18 outlook-thanks MERGED into ONE 3-part closer `17_closing-takeon-ongoing-outlook.md`. TAKE-HOMES ~6 (each closes a slide-06 anchor: one-lens↩cognitive-maps · detect→characterize↩the-gap · held-trace↩replay · enc-vs-inf↩offline-consolidation · one-operator↩epilepsy · matched-strength↩guardrail); ONGOING ~6 (per-patient traces other bands · higher-order markers · low-γ→SOZ trace-or-anchor · what simpler metrics miss · task-dynamics↔trace · β spatial signature OPEN); OUTLOOK ~6 (behaviour link · fluctuation-as-biomarker · triage→clinic · nonlinear decompositions · bigger/diverse data · **AI-inspiration closes the slide-06 opening**) + thanks footer. β delocalized — localization OPEN/ongoing, never a result. Left: Canva build (3 text columns + optional dissociation glyph + thanks). Old 17/18 SUPERSEDED, kept on disk for numbers/speech. |
| — | ~~old 17 take-homes / old 18 outlook-thanks~~ | SUPERSEDED | [x] | [x] | [x] | absorbed into the compound closer above (2026-07-16) |
