#!/usr/bin/env python3
"""audit_154 — per-node trace decomposition + anti-node characterization under rho_sym.

Migrates the per-node heterogeneity explanator (audit_144) off bare rho_split onto
the canonical, arbitrary-half-free rho_sym estimator (feedback_rho_sym_canonical_
estimator). The only change vs audit_144 is the per-pair rank concordance: it becomes
the SYMMETRIC average of both split-half arm assignments (mirrors rho_sym), applied
identically to the observed trace and to every cached matched-strength surrogate:

    c_sym = 1/2 [ rank_concordance(D_task-D_preA, D_post-D_preB)
                + rank_concordance(D_task-D_preB, D_post-D_preA) ]

Then T_i = node_incidence_mean(c_sym) exactly as audit_144 (mean_i T_i is now
proportional to rho_sym, so a low-rho_sym patient is still one whose node-mean is
dragged down by anti-trace nodes). Reuses audit_83's cached R=200 seed-20260511
surrogate cophenetic eigs (surrogate_cache_path) — NO regeneration. Does NOT edit
audit_144; new script, new output dir.

5-point critical preamble
1. Claim: the carrier/anti/neutral per-node decomposition and the anti-node
   characterization hold under rho_sym as under rho_split (heterogeneity = anti-trace
   nodes, not an arbitrary-half artifact).
2. Null (polarity): matched-strength 4-cycle ±δ surrogate (audit_63 R=200 ensemble),
   per-node aggregated identically to observed, under the SAME sym concordance.
3. Strongest alternative: node strength, controlled by the strength-preserving
   surrogate; estimator change is orthogonal to strength.
4. Cannot: reuses R=200 cache; discovery-step characterization only (does NOT run the
   closing decimation test, gated on a candidate P emerging); does NOT re-test trace
   existence (locked §5.2/5.4).
5. Falsify: if the carrier/anti split or the leading anti-node property inverts under
   rho_sym, the per-node story is estimator-dependent.

Scope report (shared with audit_144):
  .agents/guides/task-persistence-investigation/2026-06-25_per-node-trace-decomposition.md

Outputs (data/audit/per_node_trace_decomposition_rhosym/)
  per_node.csv               per (patient, band, node): T_raw, t_rel, p_plus,
                             p_minus, polarity, pi_tilde + a-priori properties
  per_patient_summary.csv    per (patient, band): rho_sym + carrier/anti/neutral counts
  characterization_{band}.csv per (property, level): anti/carrier fractions + shuffle p
"""
...
OUT = ROOT / "data/audit/per_node_trace_decomposition_rhosym"
...
# ---------------------------------------------------------------------------
# observed + surrogate SYM cophenetic trace
# ---------------------------------------------------------------------------
...
def obs_cophenet_trace(pat, band):
    """Per-pair SYM rank-concordance c_p (length P) + upper-tri indices + N + rho_sym."""
    ...
    c = _sym_concordance(DA, DB, Dt, Dpost)
    r_ab = float(spearmanr(Dt - DA, Dpost - DB).statistic)
    r_ba = float(spearmanr(Dt - DB, Dpost - DA).statistic)
    rho = 0.5 * (r_ab + r_ba)
    N = Wt.shape[0]
...
# ---------------------------------------------------------------------------
# node properties (read-only; all a-priori / trace-independent) — verbatim a144
# ---------------------------------------------------------------------------
...
def per_cell(pat, band, verbose=True):
    ...
    for r in range(R):
        if any(surco[ph][r] is None for ph in PHASES):
            continue
        cr = _sym_concordance(surco["rest_pre_A"][r], surco["rest_pre_B"][r],
                              surco["task_test"][r], surco["rest_post"][r])
        Ts.append(node_incidence_mean(cr, iu_i, iu_j, N, demean=False))
    Ts = np.vstack(Ts) if Ts else np.full((1, N), np.nan)
    Rv = Ts.shape[0]
    ...
    if verbose:
        ...
        print(f"  {pat} {band:9s}: rho_sym={rho:+.3f}  N={N}  "
              f"carrier={nc} anti={na} neutral={N-nc-na}  (Rv={Rv})")
    return props


# ---------------------------------------------------------------------------
# discovery step: characterize anti vs carrier by a-priori property — verbatim a144
# ---------------------------------------------------------------------------
...
def main():
    ...
    keep_cols = ["patient", "band", "node_idx", "label_raw", "region", "lobe",
                 ...
                 "rho_sym", "T_raw", "t_rel", "surr_mean_T", "p_plus",
