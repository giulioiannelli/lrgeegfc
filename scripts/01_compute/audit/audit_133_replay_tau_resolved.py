#!/usr/bin/env python3
"""audit_133 — N4 v3 / R1: τ-RESOLVED transient reinstatement (the collapsed scale axis).

Scope: .agents/guides/task-persistence-investigation/2026-06-22_replay-states-v3-scale-target-pair.md

Every prior replay test fixed τ = 1/λmax (finest scale) and got a NEGATIVE for transient
states. PI hypothesis: a transient replay state may hide at a MESOSCALE τ* — a coarse
community structure that snaps back transiently though the full-resolution cophenetic is
stationary. R1 sweeps τ = α/λmax per window (audit_121 reconstruction, bit-exact at α=1)
and runs the make-or-break burstiness battery PER SCALE.

Per patient, band, α (τ = α/λmax_window):
  Dᶜ_w(α) = cophenetic of windowed |ImCoh| LRG at that τ (20 s windows; feasible band).
  refs Dᶜ_T(α), Dᶜ_{R-}(α) full-phase; task-likeness contrast
    s_w(α) = ρcoph(Dᶜ_w(α), Dᶜ_T(α)) − ρcoph(Dᶜ_w(α), Dᶜ_{R-}(α))   (z-scored pooled)
  shift(α)        = mean_post − mean_pre   (= N1 level at that scale; anchor at α=1)
  AC1/excess/spread vs TIME-SHUFFLE        = transient STATES at that scale
  PLACEBO: same with Dᶜ_T node-PERMUTED (isospectral, anatomy scrambled). Coarse-τ
    collapse manufactures burstiness for ANY target → a real replay state needs
    AC1_post(real) > AC1_post(placebo), not merely > time-shuffle.

Verdict: a scale τ* clears only if cohort burstiness > time-shuffle AND target-specific
(real > placebo). Anchor: shift(α=1) must reproduce the sustained 10/10 (audit_129 A1).

Output: data/audit/replay_states/tau_resolved_{per_patient,cohort}.csv
"""
...
def main():
    ...
    df.to_csv(OUT / "tau_resolved_per_patient.csv", index=False)
    ...
    cohdf.to_csv(OUT / "tau_resolved_cohort.csv", index=False)

    print("\n=== R1 τ-RESOLVED transient — cohort τ-profile ===")
    ...
    print(f"\n[audit_133] done in {time.time()-t0:.1f}s -> {OUT}")
