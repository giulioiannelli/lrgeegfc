#!/usr/bin/env python3
"""Audit 51 -- §5.5 anatomy figure with α parsopercularis halos (round-3 disclosure).

Builds on audit_50 (3-band α + β + γ_l anatomy_2d) by adding halos around
the two-patient base of the α ctx-lh-parsopercularis cell:

  - Pat_07's 3 α parsopercularis trace-leaves: green halo
  - Pat_14's 2 α parsopercularis trace-leaves: blue halo (distinct from the
    yellow already used on Pat_02 γ_l fusiform)

The yellow γ_l halo on Pat_02 fusiform is preserved unchanged.
...
OUT_FIG = (ROOT / "data" / "outputs" / "figures" / "section_5_lrg_trace"
           / "headline" / "anatomy_2d_disclosed.pdf")
OUT_RPT = (ROOT / "data" / "reports" / "section_5_lrg_trace" / "headline"
           / "figures" / "anatomy_2d_disclosed.pdf")
...
# Halo colors: yellow already used for γ_l Pat_02; green + blue for α Pat_07/14.
HALO_PAT07_ALPHA = "#2ca25f"   # green for Pat_07 (anti-aligned, dominant)
HALO_PAT14_ALPHA = "#3182bd"   # blue  for Pat_14 (secondary contributor)
HALO_PAT02_GAMMA = "#f0c14b"   # yellow (unchanged)


# --- region-label abbreviation map (Desikan-Killiany compact form) ---------
# Cortical hemisphere prefix: ctx-lh- -> "L ", ctx-rh- -> "R "  (matches legend)
# Compound stems compressed but recognizable. Hip / Amy stay as-is.
REGION_STEM_ABBREV = {
    "superiortemporal": "sup-temp",
    "middletemporal": "mid-temp",
    "inferiortemporal": "inf-temp",
    "superiorfrontal": "sup-front",
    "rostralmiddlefrontal": "rost-mid-front",
    "caudalmiddlefrontal": "caud-mid-front",
    "lateralorbitofrontal": "lat-orb-front",
    "medialorbitofrontal": "med-orb-front",
    "parsopercularis": "parsOp",
    "parstriangularis": "parsTri",
    "rostralanteriorcingulate": "rost-ant-cing",
    "isthmuscingulate": "isth-cing",
    "posteriorcingulate": "post-cing",
...
def short_region(reg: str) -> str:
    """Abbreviate a Desikan-Killiany region label for tight bar plots."""
    if reg.startswith("ctx-lh-"):
        prefix, stem = "L ", reg[len("ctx-lh-"):]
    elif reg.startswith("ctx-rh-"):
        prefix, stem = "R ", reg[len("ctx-rh-"):]
    else:
        prefix, stem = "", reg
    return prefix + REGION_STEM_ABBREV.get(stem, stem)
...
def enrichment_for_band(df_leaf: pd.DataFrame, base: pd.DataFrame,
                        band: str) -> tuple[pd.DataFrame, float]:
...
def make_figure() -> None:
    ...
    for col_idx, band in enumerate(BANDS):
        ...
        # Set xlim first so we can decide where the in-bar rate label sits
        x_max = max(agg.enrichment.max() * 1.40, 3.5)
        ax.set_xlim(0, x_max)
        ...
        for i, (_, r) in enumerate(agg.iterrows()):
            ...
            # In-bar rate label: white text, right-aligned just inside bar end
            rate_str = f"{r.trace_rate*100:.0f}%"
            # If bar is wide enough (>= 0.55 enrichment units), put rate inside;
            # else put it just outside the bar at left of the external block.
            if r.enrichment >= 0.55:
                ax.text(r.enrichment - 0.05, i, rate_str,
                        va="center", ha="right", fontsize=7.5,
                        color="white", fontweight="bold", zorder=4)
                ext_x = r.enrichment + 0.08
            else:
                ext_x = r.enrichment + 0.08
                rate_str_inline = rate_str + "  "  # prepend to line 1
            # External 2-line annotation
            line1 = f"{int(r.n_trace)}/{int(r.n_contacts)}  ·  {int(r.n_pat_trace)} pts"
            line2 = f"p={r.p_hyper:.3f} {stars}"
            if r.enrichment < 0.55:
                line1 = rate_str_inline + line1
            ax.text(ext_x, i - 0.20, line1, va="center", fontsize=7.0,
                    color="0.15")
            ax.text(ext_x, i + 0.20, line2, va="center", fontsize=7.0,
                    color="0.30")

        ax.set_yticks(range(len(agg)))
        ax.set_yticklabels([short_region(r) for r in agg.region.tolist()],
                           fontsize=8.5)
    ...
    # ---- Bottom row: axial scatter with halos ----
    last_sc = None
    for col_idx, band in enumerate(BANDS):
        ...
        # γ_l fusiform halo (Pat_02), unchanged
        if band == "low_gamma":
            halo = sub[(sub.patient == "Pat_02") & (sub.region == "ctx-lh-fusiform")]
            if not halo.empty:
                ax.scatter(halo.x / 1000, halo.y / 1000, s=240, marker="o",
                           facecolor="none", edgecolor=HALO_PAT02_GAMMA,
                           linewidths=2.6, zorder=4)

        # α parsopercularis halos: Pat_07 (green) and Pat_14 (blue)
        if band == "alpha":
            halo07 = sub[(sub.patient == "Pat_07")
                         & (sub.region == "ctx-lh-parsopercularis")]
            if not halo07.empty:
                ax.scatter(halo07.x / 1000, halo07.y / 1000, s=240, marker="o",
                           facecolor="none", edgecolor=HALO_PAT07_ALPHA,
                           linewidths=2.6, zorder=4)
            halo14 = sub[(sub.patient == "Pat_14")
                         & (sub.region == "ctx-lh-parsopercularis")]
            if not halo14.empty:
                ax.scatter(halo14.x / 1000, halo14.y / 1000, s=240, marker="o",
                           facecolor="none", edgecolor=HALO_PAT14_ALPHA,
                           linewidths=2.6, zorder=4)
    ...
    legend_handles = [
        ...
        plt.Line2D([0], [0], marker="o", color=HALO_PAT02_GAMMA, lw=0,
                   markerfacecolor="none", markersize=11, markeredgewidth=2.3,
                   label=r"Pat_02 leaves at $\gamma_l$ fusiform"),
        plt.Line2D([0], [0], marker="o", color=HALO_PAT07_ALPHA, lw=0,
                   markerfacecolor="none", markersize=11, markeredgewidth=2.3,
                   label=r"Pat_07 leaves at $\alpha$ parsopercularis"),
        plt.Line2D([0], [0], marker="o", color=HALO_PAT14_ALPHA, lw=0,
                   markerfacecolor="none", markersize=11, markeredgewidth=2.3,
                   label=r"Pat_14 leaves at $\alpha$ parsopercularis"),
    ]
    fig.legend(handles=legend_handles, loc="lower center",
               bbox_to_anchor=(0.5, -0.025), ncol=5, frameon=False, fontsize=8.0)

    fig.subplots_adjust(left=0.115, right=0.985, top=0.95, bottom=0.10)
    OUT_FIG.parent.mkdir(parents=True, exist_ok=True)
    OUT_RPT.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(OUT_FIG, bbox_inches="tight")
    fig.savefig(OUT_RPT, bbox_inches="tight")
    plt.close(fig)
    print(f"[51] wrote {OUT_FIG}")
    print(f"[51] wrote {OUT_RPT}")
