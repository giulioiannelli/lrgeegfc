#!/usr/bin/env python3
"""Audit 68 (β) — β epi-exclusion sensitivity (matched-strength split-baseline).

β variant of audit_68_alpha_epi_exclusion. Re-runs the audit_63
split-baseline matched-strength surrogate test at β band ONLY, after
removing epileptic-zone contacts from each patient's FC matrix. Closes
C5 epi-X cophenet coverage at the second cophenet-positive band (α was
already covered by the parent audit_68); supports the methods
generalization from "is reported for α" to "is reported for the two
cophenet-positive bands α and β".

Predicate: if the β trace is genuinely in non-epi circuits, removing
epi nodes should NOT weaken the cohort signal (and may sharpen it).
If the β trace is partly carried by epi-zone activity, removing epi
nodes should weaken the cohort signal.

Inputs (reused from audit_63):
    - cached half FCs at data/cache/imcoh_halves_fc/Pat_NN/...
    - load_fc_matrix(pat, phase, "beta", "imcoh_abs") for task_test, rest_post
    - load_epileptic_nodes(pat) + channel_labels.csv for epi masking

Outputs:
    data/audit/beta_epi_exclusion/
...
OUT = ROOT / "data" / "audit" / "beta_epi_exclusion"
FIG = OUT / "figures"
...
def per_cell(pat: str, n_surr: int, swap_factor: int,
    ...
    for phase in PHASES:
        try:
            ...
            if verbose:
                print(f"[audit_68_beta] SKIP {pat}/{phase}: {e}")
            return None
        if W.shape[0] != N_full:
            if verbose:
                print(f"[audit_68_beta] SKIP {pat}/{phase}: "
    ...
    if any(D.size != Lc for D in D_obs.values()):
        if verbose:
            print(f"[audit_68_beta] SKIP {pat}: ultrametric size mismatch")
        return None
...
def write_readme(rows: list[dict], cohort: pd.DataFrame,
    ...
    lines = [
        "---",
        "name: beta_epi_exclusion",
        "scope: methods_C5_cophenet_beta_coverage",
        f"date: {time.strftime('%Y-%m-%d')}",
        "status: complete",
        "---",
        "",
        "# β epi-exclusion sensitivity (matched-strength split-baseline)",
        "",
        "**Head.** β LRG ρ_split is re-run after removing epileptic-zone "
        ...
        "",
        "## Cohort summary (epi-excluded β only)",
        "",
    ...
    lines.extend([
        ...
        "## Interpretation",
        "",
        "C5 epi-X coverage for β closes the cophenet-positive-band "
        "set under the methods §C5 directive (the parent α run "
        "covered the only other cophenet-positive band). The "
        "predicate is: if β reorganization lives in clinically "
        "healthy circuits, removing epi nodes from the FC should "
        "leave the cohort signal intact or strengthen it; if β "
        "reorganization is partially carried by epi-zone activity, "
        "removing epi nodes should weaken the cohort signal. The "
        f"result is that the cohort median {direction} ({delta:+.3f}), with "
        ...
        "- Build script: "
        "`scripts/01_compute/audit/audit_68_beta_epi_exclusion.py`",
        "",
        "## Files",
        "",
        "- `per_patient.csv` — per-patient observed + surrogate stats",
...
def main() -> None:
    ...
    print("[audit_68_beta] pre-flight: ensure half FCs cached for α")
    for pat in patients:
    ...
    for pat in patients:
        ...
        rows.append(r)
        dt = time.time() - t_cell
        print(f"[audit_68_beta] {pat}: N_full={r['N_full']} keep={r['N_keep']} "
    ...
    print(f"[audit_68_beta] total: {runtime:.1f}s for {len(rows)} cells")
    ...
    print(f"[audit_68_beta] outputs at {OUT}")
