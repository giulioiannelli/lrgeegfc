#!/usr/bin/env python3
"""Audit 48 — KC reset modules: rPre+rPost coherent, tt fragmented.

Symmetric inverse of audit_47. A reset module is a leaf set L_pre that
exists as a coherent subtree in BOTH rest phases (rPre and rPost) but
fragments during the task. Anchored at rPre subtrees:

  Gate 1: rPost match   J(L_pre, rPost) >= J_PERSIST = 0.6
  Gate 2: tt fragment   forall tt subtree T: J(L_pre, T) < J_DISRUPT = 0.5
  Gate 3: tt containment |C_tt(L_pre)| >= FRAG_FACTOR * |L_pre|
  Gate 5: symmetric     forall tt subtree T: J(L_post, T) < J_DISRUPT

Renders Option C (3x3 dendrogram + network paired panels):
rows = lambda regime (0 / 0.5 / 1), cols = phase (rest_pre / task / rest_post).

Phase semantics relative to audit_47:
  rPre  carries the coherent anchor subtree (was tt in audit_47)
  rPost carries the matched coherent subtree (same role)
  tt    carries scattered leaf bars showing fragmentation (was rPre in audit_47)

Empty-cell skip: if no modules at lambda=0, no figure is rendered.
Linear y-axis on dendrograms.

Outputs
-------
``data/audit/kc_reset_module_view/figures/{pat}__{band}__option_C.pdf``
``data/audit/kc_reset_module_view/reset_subtrees_summary.csv``

Scope: ``.agents/guides/task-persistence-investigation/2026-05-07_kc-reset-modules.md``
...
J_PERSIST = 0.6     # rPre-rPost Jaccard threshold (mirrors trace J_POST_MIN)
J_DISRUPT = 0.5     # tt-side Jaccard upper bound (fragmentation)
FRAG_FACTOR = 2.0   # smallest tt subtree containing L_pre must be >= FRAG_FACTOR * |L_pre|
HEIGHT_REL_THR = 0.30
MIN_SIZE = 3
MAX_RESET_TOTAL = 12
...
OUT = ROOT / "data" / "audit" / "kc_reset_module_view"
FIG = OUT / "figures"
GRAY_NODE = "#cfcfcf"
...
def find_reset_pairs(
    Z_pre: np.ndarray,
    Z_post: np.ndarray,
    Z_tt: np.ndarray,
    n: int,
    *,
    j_persist: float = J_PERSIST,
    j_disrupt: float = J_DISRUPT,
    min_size: int = MIN_SIZE,
    max_size: int = None,
) -> dict:
    """Find (rPre-subtree, rPost-subtree) pairs that fragment in tt.

    Anchored at rPre (the coherent phase that the task disrupts).
    """
    if max_size is None:
    ...
    for st_pre in sts_pre:
        ...
        for st_post in sts_post:
            ...
            if u == 0:
                continue
            j = len(L_pre & L_post) / u
            if j > best_j_post:
                best_j_post = j
                best_st_post = st_post

        if best_j_post < j_persist or best_st_post is None:
            continue

        # Gate 2: tt fragmentation, leaf-set side (no size restriction).
        # Mirrors the anti-anchor fix from audit_47.
        j_tt_pre = 0.0
        for st_tt in sts_tt:
            ...
            if j > j_tt_pre:
                j_tt_pre = j
        if j_tt_pre >= j_disrupt:
            continue

        # Gate 3: tt containment fragmentation. Mirror the FRAG_FACTOR guard.
        smallest_containing_tt = float("inf")
        for st_tt in sts_tt:
            if L_pre <= st_tt["leaves"]:
                if st_tt["size"] < smallest_containing_tt:
                    smallest_containing_tt = st_tt["size"]
        if smallest_containing_tt < FRAG_FACTOR * s:
            continue

        # Gate 5: symmetric — rPost match must also fragment in tt.
        L_post = frozenset(best_st_post["leaves"])
        j_tt_post = 0.0
        for st_tt in sts_tt:
            L_tt = frozenset(st_tt["leaves"])
            u = len(L_post | L_tt)
        ...
        candidates.append({
            ...
            "j_tt_pre": j_tt_pre,
            "j_tt_post": j_tt_post,
            "size": max(len(L_pre), len(best_st_post["leaves"])),
            "score": best_j_post * (1.0 - max(j_tt_pre, j_tt_post)),
        })
    ...
    used = set()
    for c in candidates:
        union_leaves = c["leaves_pre"] | c["leaves_post"]
        ovl = len(union_leaves & used) / max(len(union_leaves), 1)
        if ovl < MAX_OVERLAP_FRAC:
            selected.append(c)
            used |= union_leaves
        if len(selected) >= MAX_RESET_TOTAL:
            break

    for i, c in enumerate(selected):
        c["color_idx"] = i

    reset_topo = list(selected)
    reset_height = []
    for c in selected:
        h_max = max(c["h_pre"], c["h_post"])
        if h_max > 0 and abs(c["h_pre"] - c["h_post"]) / h_max <= HEIGHT_REL_THR:
            reset_height.append(c)

    return {
        0.0: reset_topo,
        0.5: list(reset_height),
        1.0: reset_height,
    }
...
def module_leaves_for_phase(module: dict, phase: str) -> set:
    ...
    rPre + rPost dendrograms each show their own coherent subtree;
    tt dendrogram shows the union scattered as leaf bars."""
    if phase == "rest_pre":
...
def draw_dendro(ax, Z, modules, phase, pal, n, *, draw_height_bars=False):
    ...
    # Merge-height bars at lambda = 0.5 / 1: full-width dashed line at h_pre
    # in rPre dendrogram, h_post in rPost dendrogram. Skip tt (no anchor).
    if draw_height_bars and phase != "task_test":
        for m in modules:
            h = m["h_pre"] if phase == "rest_pre" else m["h_post"]
...
def render_option_c(patient, band, Z_by_phase, A_by_phase, pos,
    ...
    n_reset = {lam: len(reset_by_lambda[lam]) for lam in LAMBDAS}
    fig.text(
        0.5, 0.99,
        f"{patient}  {band_tex}  reset modules (Jaccard$_{{\\rm post}}$$\\geq${J_PERSIST}, "
        f"Jaccard$_{{\\rm tt}}$$<${J_DISRUPT})  "
        rf"$\lambda$=0:{n_reset[0.0]}, "
        rf"0.5:{n_reset[0.5]}, "
        rf"1:{n_reset[1.0]}",
    ...
    for patient in PATIENTS:
        for band in BANDS:
            try:
                ...
                reset_by_lambda = find_reset_pairs(
                    Z_by_phase["rest_pre"],
                    Z_by_phase["rest_post"],
                    Z_by_phase["task_test"],
                    n,
                )
                modules = reset_by_lambda[0.0]

                if not modules:
                    print(f"[audit_48] {patient} {band} -> NO modules; skipping figure")
                    summary_rows.append({
                        "patient": patient,
                        "band": band,
                        "n_nodes": n,
                ...
                # Layout seeded with rPre partition (the anchor phase for reset).
                ...
                summary_rows.append({
                    ...
                    "band": band,
                    "n_nodes": n,
                    "n_modules_lambda0": len(reset_by_lambda[0.0]),
                    "n_modules_lambda1": len(reset_by_lambda[1.0]),
                    "n_modules_lambda05": len(reset_by_lambda[0.5]),
                    "sizes_pre": sorted([len(m["leaves_pre"]) for m in reset_by_lambda[0.0]]),
                    "sizes_post": sorted([len(m["leaves_post"]) for m in reset_by_lambda[0.0]]),
                    "j_post": [round(m["j_post"], 3) for m in
                               sorted(reset_by_lambda[0.0], key=lambda x: -x["score"])],
                    "j_tt": [round(max(m["j_tt_pre"], m["j_tt_post"]), 3) for m in
                             sorted(reset_by_lambda[0.0], key=lambda x: -x["score"])],
                    "option_C": str(pc.relative_to(ROOT)),
                })
                print(
                    f"[audit_48] {patient} {band} -> reset modules topo={len(reset_by_lambda[0.0])} "
                    f"height={len(reset_by_lambda[1.0])} "
                    f"sizes_pre={sorted([len(m['leaves_pre']) for m in reset_by_lambda[0.0]])} "
                    f"j_post={[round(m['j_post'],2) for m in sorted(reset_by_lambda[0.0], key=lambda x: -x['score'])]}"
                )
            except Exception as e:
                print(f"[audit_48] WARN {patient} {band}: {e}")
                traceback.print_exc()

    df = pd.DataFrame(summary_rows)
    df.to_csv(OUT / "reset_subtrees_summary.csv", index=False)
    print(f"[audit_48] summary: {OUT / 'reset_subtrees_summary.csv'}")
