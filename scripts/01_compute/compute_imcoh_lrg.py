#!/usr/bin/env python3
"""Compute LRG analysis on ImCoh (imaginary coherence) FC matrices.

Reads ImCoh matrices from data/imcoh_cache/, runs LRG hierarchical
decomposition, and writes results to data/imcoh_lrg_cache/.

Run:
  python scripts/py/compute_imcoh_lrg.py [--patients Pat_02] [--phases rest_pre] [-v]
  python scripts/py/compute_imcoh_lrg.py --verbose       # all patients, all phases, all bands
"""
from __future__ import annotations

import argparse
import time
from pathlib import Path

import numpy as np

from lrg_eegfc.utils.scripting import setup_script_env
ROOT = setup_script_env()

from lrg_eegfc.config.const import BRAIN_BANDS, PATIENTS_4PHASE, PHASE_LABELS, nperseg_for_fs
from lrg_eegfc.config.paths import IMCOH_LRG_CACHE
from lrg_eegfc.workflow.fc import load_fc_matrix
from lrg_eegfc.workflow.lrg import compute_lrg_analysis

from lrg_eegfc.config.const import FS_OVERRIDES as FS_MAP  # canonical


def main():
    parser = argparse.ArgumentParser(
        description="Compute LRG analysis on ImCoh FC matrices."
    )
    parser.add_argument("--patients", nargs="+", default=PATIENTS_4PHASE,
                        help="Patient IDs (default: all 5)")
    parser.add_argument("--phases", nargs="+", default=list(PHASE_LABELS),
                        help="Recording phases (default: all 4)")
    parser.add_argument("--bands", nargs="+", default=list(BRAIN_BANDS.keys()),
                        help="Frequency bands (default: all 6)")
    parser.add_argument("--overwrite", action="store_true",
                        help="Recompute even if LRG cache exists")
    parser.add_argument("--verbose", "-v", action="store_true",
                        help="Print progress information")
    args = parser.parse_args()

    total_t = time.time()
    n_done = 0
    n_skip = 0
    n_fail = 0

    for pat in args.patients:
        fs = FS_MAP.get(pat, 2048.0)
        nperseg = nperseg_for_fs(fs)

        print(f"\n{'='*60}")
        print(f"{pat}  (fs={fs:.0f} Hz, nperseg={nperseg})")
        print(f"{'='*60}")

        for phase in args.phases:
            for band in args.bands:
                # Check if LRG already cached. The loader encodes the
                # transform in the filename so |ImCoh| and |ImCoh|² can
                # coexist under IMCOH_LRG_CACHE.
                out_dir = IMCOH_LRG_CACHE / pat
                out_path = out_dir / f"{band}_{phase}_lrg_imcoh-abs.npz"
                if out_path.exists() and not args.overwrite:
                    if args.verbose:
                        print(f"  {phase}/{band}: LRG cached, skipping")
                    n_skip += 1
                    continue

                # Load |ImCoh| = mean(|signed|, axis=F_band) via the new
                # dispatcher (reads freq-resolved signed cache, applies
                # np.abs per-bin then band-averages).
                A = load_fc_matrix(pat, phase, band, "imcoh_abs")
                if A is None:
                    if args.verbose:
                        print(f"  {phase}/{band}: no ImCoh cache, skipping")
                    n_skip += 1
                    continue
                t0 = time.time()
                np.fill_diagonal(A, 0)

                try:
                    result = compute_lrg_analysis(
                        A,
                        patient=pat,
                        phase=phase,
                        band=band,
                        fc_method="imcoh_abs",
                        use_cache=False,   # we manage our own cache
                    )
                except Exception as e:
                    print(f"  {phase}/{band}: FAILED -- {e}")
                    n_fail += 1
                    continue

                # Save to our own cache directory
                out_dir.mkdir(parents=True, exist_ok=True)
                np.savez_compressed(
                    out_path,
                    ultrametric_matrix=result.ultrametric_matrix,
                    linkage_matrix=result.linkage_matrix,
                    entropy_tau=result.entropy_tau,
                    entropy_1_minus_S=result.entropy_1_minus_S,
                    entropy_C=result.entropy_C,
                    optimal_threshold=result.optimal_threshold,
                    patient=pat,
                    phase=phase,
                    band=band,
                    fc_method="imcoh_abs",
                    n_nodes=result.n_nodes,
                )

                dt = time.time() - t0
                if args.verbose:
                    print(f"  {phase}/{band}: n={result.n_nodes}, "
                          f"threshold={result.optimal_threshold:.4f}, "
                          f"{dt:.1f}s")
                n_done += 1

    elapsed = time.time() - total_t
    print(f"\nDone: {n_done} computed, {n_skip} skipped, {n_fail} failed "
          f"in {elapsed:.0f}s")
    print(f"Output: {IMCOH_LRG_CACHE}")


if __name__ == "__main__":
    main()
