#!/usr/bin/env python3
r"""fig:hierarchy-chord-network (mst@0.20 backbone) — radial bundled network + branch-origin
hierarchy overlay, rebuilt on the **sparsified multiscale backbone** at a single diffusion
scale s = tau*lambda_max ~ 5.65.

FORK of ``figures_embedded/fig_hierarchy_chord_network.py``. The ONLY change is the tree
substrate that both the Holten bundling routes THROUGH and the branch-origin dendrogram
overlay are drawn from: the dense single-scale LRG tree (``tau = 1/lambda_max`` on the
...
Reuses the attribution helpers (``attribute`` / ``link_color`` / ``leaf_origin_weights``)
from ``fig_branch_origin_tree`` (DRY).
Writes: data/preprint/figures/new_results_sec1/_drafts/
        fig_hierarchy_chord_network_{pat}_{band}_{phase}_mst020_DRAFT.pdf
"""
...
from fig_branch_origin_tree import (                                   # type: ignore
    ...
    AMBER, TEAL, GREY, PHASES,                      # PHASES = 4-phase attribution list
)
...
def reference_scales(ref_pat, ref_band):
    ...
    (mst@0.20 fork: the dense ``lrg_linkage`` / ``coph_square`` are swapped for the scale-
    addressed ``tree_at_scale`` / ``coph_square_at_scale``.)"""
    Zr = tree_at_scale(load_phase(ref_pat, "rest_post", ref_band), S_REPORT)
    Nr = Zr.shape[0] + 1
    cophr = {ph: coph_square_at_scale(load_phase(ref_pat, ph, ref_band), S_REPORT)
             for ph in PHASES}
...
def build(pat, band, phase, k_comm, topk, beta, gamma, size, node_color, edge_cmap,
    ...
    # mst@0.20 backbone tree at the reporting scale (drop-in for lrg_linkage) ---
    Z = tree_at_scale(load_phase(pat, phase, band), S_REPORT)
    ...
    def render(cr, W):
        ...
        def cbar(x, cmap, lo, hi, title, left, right):
            ...
            for i in range(25):
                t = i / 24.0
                col = cmap(lo + (hi - lo) * t)
                grad.add_color_stop_rgb(t, col[0], col[1], col[2])
    ...
    stem = f"fig_hierarchy_chord_network_{pat}_{band}_{phase}{tag}_mst020_DRAFT"
    surf = cairo.PDFSurface(str(DRAFTDIR / f"{stem}.pdf"), float(size), float(size) * (1 + STRIP))
