...
-> heat kernel -> UPGMA cophenetic at scale s -> rho_sym, swept over s.
...
Outputs (data/paper_final/w0a_substrate/a2_stability/{transform}/):
  per_patient_scale.csv  config, method, param, patient, band, s, obs_rho, surr_p50/p95, p
  cohort_gate.csv        config, method, param, band, s, gate_p, n_above, obs_med, surr_med
  structure.csv          config, patient, band, phase, density, clustering, lambda_2, ...
  config.json
"""
...
def load_phase(pat, phase, band):
    """Four-phase FC under the selected transform. A/B are the rest_pre halves."""
    if phase in ("A", "B"):
...
def per_cell(job):
    ...
    surr = {label: np.full((R, SGRID.size), np.nan) for label, _, _ in CONFIGS}
    for r in range(R):
        ...
        for label, method, param in CONFIGS:
            eig_s = {ph: laplacian_eig(make_backbone(Wsh[ph], method, param))
                     for ph in PHASES}
            surr[label][r] = rho_sym_over_scales(eig_s, SGRID)

    rows = []
    for label, method, param in CONFIGS:
        o_all, s_all = obs[label], surr[label]
        for j, s in enumerate(SGRID):
            ...
            rows.append(dict(config=label, method=method, param=param,
                             patient=pat, band=band, s=float(s), N=int(N),
...
def cohort_gate(df, bands):
    """Per (config, band, scale) one-sided Wilcoxon on obs - surr_p50 over patients."""
    out = []
        ...
        for band in bands:
            ...
            for s in np.sort(d1.s.unique()):
                ...
                try:
                    ...
                    p = np.nan
                out.append(dict(config=label, method=method, param=param, band=band,
                                s=float(s), gate_p=p, n_above=int((x.p < 0.05).sum()),
...
def main():
    ...
    print(f"\n=== n_scales cleared (gate_p < 0.05) out of {len(SGRID)}, per config x band ===",
          flush=True)
    hdr = "  " + "config".ljust(16) + "".join(b[:5].rjust(8) for b in bands) + "   density"
    print(hdr, flush=True)
    st = pd.DataFrame(srows)
    for label, _, _ in CONFIGS:
        g = gate[gate.config == label]
        if g.empty:
            continue
