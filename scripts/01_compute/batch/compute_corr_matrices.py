...
Usage:
    ...
    python scripts/py/compute_corr_matrices.py

    # Raw correlation (keep negative values)
    python scripts/py/compute_corr_matrices.py --filter-type none

    # Specific patients
    python scripts/py/compute_corr_matrices.py --patients Pat_02 Pat_03

    # Custom filter order
    python scripts/py/compute_corr_matrices.py --filter-order 6 --verbose
"""
...
from lrg_eegfc.config.paths import CORR_CACHE, CORR_DEV_CACHE
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=CORR_CACHE,
        help="Cache directory (default: data/cache/corr)"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    if args.filter_time is not None and args.filter_time > 0 and args.cache_root == CORR_CACHE:
        args.cache_root = CORR_DEV_CACHE
