...
def vi_per_patient_clean_fraction() -> pd.DataFrame:
    """For every (patient, band) the fraction of clean cells where T_VI > 0 (trace)."""
    rows = []
...
def grassmann_brief() -> pd.DataFrame:
            ...
            try:
                ...
                for k in K_NEW:
                    ...
                    pt.append({
                        ...
                        # T_E1 > 0 = trace (rsPost closer to task than rsPre).
                        "T_E1": chordal(Vp, Vt) - chordal(Vt, Vq),
                    })
            except Exception as e:
    ...
    ax_l.set_ylabel(r"$T_{E1}$ — positive = trace")
...
def pat03_check() -> None:
    ...
    try:
        _, p_full = wilcoxon(full, alternative="greater")
    except Exception:
        p_full = float("nan")
    try:
        _, p_drop = wilcoxon(drop, alternative="greater")
    except Exception:
        p_drop = float("nan")
