...
OUTPUT_DIR = ROOT / "data" / "figures" / "phase_reorganization" / PATIENT
...
PHASE_SHORT = {"rsPre": "rsPre", "taskLearn": "tLearn", "taskTest": "tTest", "rsPost": "rsPost"}
...
for row_idx, band in enumerate(DENDRO_BANDS):
    ...
    if "rsPre" in partitions:
        ref = partitions["rsPre"]
...
try:
    ...
    # Get theta partitions (aligned to rsPre)
    theta_partitions = {}
    theta_nstars = {}
    for phase in PHASES:
        ...
        theta_nstars[phase] = n_star

    if "rsPre" in theta_partitions:
        ref = theta_partitions["rsPre"]
        for phase in PHASES[1:]:
            if phase in theta_partitions:
                theta_partitions[phase] = align_labels_hungarian(ref, theta_partitions[phase])

    n_lrg = len(theta_partitions.get("rsPre", []))
    if n_lrg < N:
    ...
    transitions = [
        ("rsPre", "taskLearn", "rsPre -> tLearn"),
        ("taskLearn", "taskTest", "tLearn -> tTest"),
        ("taskTest", "rsPost", "tTest -> rsPost"),
    ]
