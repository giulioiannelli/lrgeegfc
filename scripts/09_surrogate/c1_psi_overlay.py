...
from lrg_eegfc.config.paths import MSC_CACHE, TABLES_ROOT, FIGURES_ROOT
...
threshold_data = dict(
    np.load(TABLES_ROOT / "threshold_analysis_Pat02.npz", allow_pickle=True)
)
percentiles = threshold_data["percentiles"]

traj_data = dict(
    np.load(TABLES_ROOT / "mslcd_coarsening_trajectories.npz", allow_pickle=True)
)
...
with plt.rc_context(_RC):
    ...
    # -- Save -------------------------------------------------------------------
    out = (
        FIGURES_ROOT / "report_mslcd_section" / "threshold_analysis"
